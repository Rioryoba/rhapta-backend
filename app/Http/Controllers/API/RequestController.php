...
class RequestController extends Controller
{
	...
	public function index()
	{
		...
		if ($role === 'manager' && $employee) {
			...
			// Show all fund requests from staff - accountants can see all requests
			// This gives them full visibility into all financial requests
		} else {
			$query->where('employee_id', $employee?->id);
		}
		$requests = $query->with(['employee', 'department', 'currentApprover'])->orderBy('created_at', 'desc')->paginate();
